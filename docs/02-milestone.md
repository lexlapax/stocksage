...
```
python -m cli.main leaderboard [--by accuracy|alpha|count] [--min-resolved N]
```
...
- [ ] `python -m cli.main summary AAPL` shows accuracy stats, trend line, per-rating breakdown
- [ ] `python -m cli.main leaderboard` ranks all tickers with resolved outcomes
- [ ] `python -m cli.main models` shows per-model accuracy
- [ ] `python -m cli.main resolve` resolves in a single yfinance batch call
- [ ] `pytest tests/` passes with >90% coverage on `core/trends.py`
- [ ] `get_cross_ticker_lessons()` returns formatted string usable as LLM prompt context
...
cli/
└── main.py                 ← add leaderboard, models commands; enhance summary
